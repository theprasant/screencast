...
function connect() {
  ...
  connection.onmessage = (e) => {
    ...
    switch (msg.type) {
      case "id":
        clientID = msg.id;
        let uname = prompt("Choose a username", "arnav");
        setUsername(uname);
        text += `[${timeStr}] Setting Username...<br>`;
        break;

      case "username":
        text += `[${timeStr}] Logged in as <b>${msg.name}</b><br>`;
        break;
      ...
      case "rejectusername":
        text += `[${timeStr}] Error: Username Not Available!<br>`;
        break;

      case "userList":
        printOnlineUsers(msg);
        break;
...
function printOnlineUsers(msg) {
  if (msg.users[0] == null) return;
  var listElem = document.getElementById("online");
...
function sendData(msg) {
  log(`Sending "${msg.type}" message: ${JSON.stringify(msg, null, 2)}`);
...
async function invite(evt) {
  ...
  if (myPeerConnection) {
    ...
    let callMode = document.getElementById("call_mode").value;
    try {
      webcamStream = await navigator.mediaDevices[`get${callMode}Media`](mediaConstraints);
